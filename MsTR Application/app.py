...
# # load in app data
file_name = drivestem + '/AAOS/MsTR/Data/app_data_final.pkl'
...
index_page = html.Div([
    html.H1('MGB Spine Tumor Dashboard', style={'font-family' : 'Geneva','padding' : '0px 30px', 'font-size' : '60px', 'text-decoration': 'bold',
...
dropdown_headers = html.Div(
    html.Div(className = 'row', children = [
        ...
        html.Div([html.Label(['Division:'], style={'text-align':'center', 'padding-left':'5px'}), 
                  dcc.Dropdown(
                      options=[{'label': 'Spine Tumor', 'value': 'AJRR'}],
                      value='AJRR', id='div_dd', multi=False, clearable = False)
                  ],style={'padding-left':'10px','padding-right':'10px','width':'12.5%',
                           'font-family':'sans-serif'}),
...
page_1_layout = html.Div([
            ...
            html.Div([
                ...
                html.H4('MGB Spine Tumor Dashboard', style={'font-family' : 'GENEVA','padding' : '0px 10px', 'font-size' : '40px', 'text-decoration': 'bold', 
...
def update_pat_info(username, provider, inst, diag, site, proc, start_date, end_date):
    if username in USER_TO_NAME.keys():
        try: 
            ...
            (total_proc, avg_length_of_stay, stdev_len_of_stay, BMI_total, avg_pat_age, stdev_age)= nongraph(data)#, preop_proms_pts, postop_proms_pts, preop_proms_perpt,
...
def update_graphs(username, provider, inst, diag, site, proc, start_date, end_date):
    if username in USER_TO_NAME.keys():
        try: 
            ...
            #CREATE GRAPHS
            (gender_graph, pat_age_bar, diag_bar, proc_bar, CCI_bw, proc_revision_pie, tob_use_bar, discharge_distr_pie, comorb_bar) = create_current_graphs(data, dateless_data, start_date, end_date)#, linked_bar, readmit_diags_bar) = create_current_graphs(data, dateless_data, start_date, end_date)
            # df2 = px.data.election() # replace with your own data source
            # geojson = px.data.election_geojson()
            # fig = px.choropleth(
            #     df2, geojson=geojson, 
